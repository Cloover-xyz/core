...
abstract contract ClooverRaffleInternal is ClooverRaffleStorage {
    ...
    function _purchaseTickets(uint16 nbOfTickets) internal {
        ...
        if(maxTicketAllowedToPurchase > 0 && _participantInfoMap[msg.sender].nbOfTicketsPurchased + nbOfTickets > maxTicketAllowedToPurchase)
    ...
    ) internal view returns (uint256 creatorAmount, uint256 protocolFeesAmount, uint256 royaltiesAmount)
    {
    ...
    function _handleCreatorInsurance() internal returns(uint256 treasuryAmountToTransfer, address treasuryAddress){
        ...
        (treasuryAmountToTransfer, ) = ticketSalesInsurance.splitInsuranceAmount(
            ...
            _config.protocolFeeRate,
            currentSupply,
            _config.ticketPrice
        );
        treasuryAddress = _config
    ...
    function _calculateInsuranceCost() internal view returns (uint256 insuranceCost) {
        ...
        insuranceCost = _config.ticketSalesInsurance.calculateInsuranceCost(
                _config.insuranceRate,
                _config.ticketPrice
            );
    }
    ...
    function findUpperBound(ClooverRaffleTypes.PurchasedEntries[] memory array, uint256 ticketNumberToSearch) internal pure returns (uint256) {
